...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    w1 = tf.Variable(tf.truncated_normal([X.get_shape().as_list()[1], hiddensize],stddev=0.1))
    b1 = tf.Variable(tf.ones([hiddensize])/10)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
  ...
  # Dense Layer
    pool2_flat = tf.reshape(conv2, [-1,28*28*10])
