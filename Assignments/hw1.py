...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    :param X: The  input placeholder for limages from the MNIST dataset
    :param Y: The output placeholder for image labels
    :return: The following variables should be returned in the following order.
        ...
        batch_loss: The average cross-entropy loss of the batch
    """
    w1 = tf.Variable(tf.truncated_normal([X.get_shape().as_list()[1], hiddensize], mean=0.0, stddev=0.1, dtype=tf.float32))
    b1 = tf.Variable(tf.zeros([hiddensize])/10)
    y1 = tf.nn.sigmoid(tf.matmul(X, w1) + b1)
    
    w2 = tf.Variable(tf.truncated_normal([hiddensize, outputsize], mean=0.0, stddev=0.1,dtype=tf.float32))
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    #Call one layer funcyion
    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(pool2_flat,Y)
